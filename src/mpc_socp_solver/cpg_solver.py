...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    if updated_params is None:
        updated_params = ["param3412", "param3405", "param3406", "param3409", "param3410", "param3407", "param3408"]
    ...
    par.param3412 = list(param_dict["param3412"].value.flatten(order="F"))
    par.param3405 = list(param_dict["param3405"].value.flatten(order="F"))
    par.param3406 = list(param_dict["param3406"].value.flatten(order="F"))
    par.param3409 = list(param_dict["param3409"].value.flatten(order="F"))
    par.param3410 = list(param_dict["param3410"].value.flatten(order="F"))
    par.param3407 = list(param_dict["param3407"].value.flatten(order="F"))
    par.param3408 = list(param_dict["param3408"].value.flatten(order="F"))
    ...
    prob.var_dict['var3403'].save_value(np.array(res.cpg_prim.var3403).reshape((14, 30), order='F'))
    prob.var_dict['var3404'].save_value(np.array(res.cpg_prim.var3404).reshape((3, 29), order='F'))
...
def cpg_gradient(prob, gradient_sol_primal=None, gradient_sol_dual=None):
    ...
    if gradient_sol_primal is not None and gradient_sol_dual is not None:
        ...
        gradient_sol.primal = [0] * 1087
        gradient_sol.dual = [0] * 1348
        use_sol = False

    # set variable deltas
    vdelta = cpg_module.mpc_socp__cpg_vdelta()
    vdelta.var3403 = list(prob.var_dict["var3403"].gradient.flatten(order="F"))
    vdelta.var3404 = list(prob.var_dict["var3404"].gradient.flatten(order="F"))
    pdelta = cpg_module.gradient(vdelta, gradient_sol, use_sol)
    prob.param_dict['param3412'].gradient = np.array(pdelta.param3412).reshape((14, 29), order='F')
    prob.param_dict['param3405'].gradient = np.array(pdelta.param3405).reshape(14)
    prob.param_dict['param3406'].gradient = np.array(pdelta.param3406).reshape((14, 30), order='F')
    prob.param_dict['param3409'].gradient = np.array(pdelta.param3409).reshape((14, 406), order='F')
    prob.param_dict['param3410'].gradient = np.array(pdelta.param3410).reshape((14, 87), order='F')
    prob.param_dict['param3407'].gradient = np.array(pdelta.param3407).reshape((3, 29), order='F')
    prob.param_dict['param3408'].gradient = np.array(pdelta.param3408).reshape(29)
